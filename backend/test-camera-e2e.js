// test-camera-e2e.js - End-to-end Camera Subsystem Verification
...
async function runTests() {
  ...
  }, { email: 'rider@smartbike.io', password: 'password123' });
  ...
  if (loginRes.statusCode === 200 && loginRes.body.token) {
    ...
    // Try registering or demo login
    console.log('Login failed with', loginRes.statusCode, loginRes.body, '- registering test rider...');
  ...
  if (!snapRes.body.success || !snapRes.body.event || !snapRes.body.image) {
    throw new Error('Snapshot failed: ' + JSON.stringify(snapRes.body));
  }
  console.log(`✓ Real Snapshot captured! Base64 length: ${snapRes.body.image.length} chars`);
  console.log(`✓ Media Reference stored: ${snapRes.body.event.mediaReference}`);

  const snapshotEventId = snapRes.body.event.eventId;
